...
const ACHIEVEMENTS_LIST: AchievementDef[] = [
  { id: "first-step", iconName: "first-step" },
  { id: "task-master", iconName: "task-master" },
  { id: "pomodoro-pioneer", iconName: "pomodoro-pioneer" },
  { id: "focus-guru", iconName: "focus-guru" },
  { id: "streak-starter", iconName: "streak-starter" },
  { id: "streak-legend", iconName: "streak-legend" },
  { id: "high-priority", iconName: "high-priority" },
  { id: "level-up", iconName: "level-up" },
];
...
export default function GamificationPanel() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold tracking-tight">{t("gamification.title")}</h2>
      ...
      <Card className="bg-card/60 backdrop-blur-md border relative overflow-hidden">
        ...
        <CardContent className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h3 className="text-lg font-bold">{t("dashboard.plannerLevel", { level: level.toString() })}</h3>
              <p className="text-xs text-muted-foreground font-medium mt-0.5">
                {t("gamification.levelDesc")}
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 max-w-md w-full space-y-2">
            <div className="flex justify-between text-xs font-bold font-mono">
              <span className="text-muted-foreground">{t("gamification.progression")}</span>
              <span>
            ...
            <p className="text-[10px] text-muted-foreground/80 font-medium">
              {t("gamification.xpNeeded", { xp: (xpNeeded - xp).toString(), nextLevel: (level + 1).toString() })}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b pb-2">
          <h3 className="text-md font-bold tracking-tight">
            {t("gamification.achievementsCount", { unlocked: unlockedAchievements.length.toString(), total: ACHIEVEMENTS_LIST.length.toString() })}
          </h3>
                ...
                <CardContent className="p-4 flex flex-col items-center text-center space-y-3.5 relative">
                  ...
                  <div>
                    <h4 className={`text-sm font-bold tracking-tight ${isUnlocked ? "text-foreground" : "text-muted-foreground"}`}>
                      {t(`gamification.achievement.${achievement.id}.title`)}
                    </h4>
                    <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed max-w-[160px] mx-auto">
                      {t(`gamification.achievement.${achievement.id}.desc`)}
                    </p>
                  </div>
